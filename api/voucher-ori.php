...
if(isset($_GET['packet'])){
    $checks = (empty($_GET['packet']) ? " and defaults = 'true'" : " and groupname = '".$_GET['packet']."'");
    $packet = $Bsk->Tampil("packet", "id, groupname as profile, voucher", "identity = '$Menu[identity]' and client = '$Menu[id]' and status = 'true' ".$checks);
    echo json_encode($packet ? 
		array("status" => true, "message" => "success", "data" => $packet) : 
		array("status" => false, "message" => "error", "data" => false), true
	);
}
if(isset($_GET['profiles'])){
    ...
    $query_profile = $Bsk->View(
        "radgroupcheck a left join radgroupreply b on a.groupname = b.groupname inner join packet c on a.groupname = c.groupname", 
        "a.groupname as id, a.groupname as name, a.groupname", 
        "a.identity = '$Menu[identity]' and c.client = '$Menu[id]' GROUP BY a.groupname", "a.groupname asc"
    );
...
if(isset($_POST['qty'])){
    ...
    $voucer = $Bsk->Tampil("packet", "id, voucher", "identity = '$Menu[identity]' and client = '$Menu[id]' and groupname ='$profil' and status = 'true'");
    if($voucer['voucher'] >= $qty){
        for($i=0; $i<$qty; $i++){
        ...
        $generate = $Bsk->Ubah("packet", array("voucher" => ($voucer['voucher'] - $qty)), "id = '$voucer[id]'");
    } else {
        $generate = false;
    }
