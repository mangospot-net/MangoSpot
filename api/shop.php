<?php
$search = $Bsk->Tampil("users a inner join level b on a.level = b.slug", "a.id", "a.identity = '$Menu[identity]' and b.id = '$Menu[level]'");
...
if(isset($_GET['price'])){
    ...
    $price = $Bsk->Tampil("packet", "price", "identity = '$Menu[identity]' and users = '$admins' and client = '$Menu[id]' and id = '$idprc' and status = 'true'");
...
if(isset($_GET['detail'])){
    ...
    $query_detail = $Bsk->Tampil("payment", "*", "id = '$id_detail' and identity = '$Menu[identity]' and users = '$admins' and client = '$Menu[id]'");
...
if(isset($_POST['packet'])){
    ...
    unset($_POST['id']);
    $check_post = $Bsk->Tampil("payment", "id", "id = '$id_post' and identity = '$Menu[identity]' and users = '$admins' and client = '$Menu[id]'");
    $query_post = ($check_post ? 
        $Bsk->Ubah("payment", $_POST, "id = '$check_post[id]' and (status = 'false' or status is null)") : 
        $Bsk->Tambah("payment", 
            array_merge(
                $_POST, 
                array(
    ...
    $packets = $Bsk->Tampil("packet", "groupname, price", "client = '$Menu[id]' and id = '".$_POST['packet']."'");
...
if(isset($_POST['delete'])){
    $query_delete = $Bsk->Hapus("payment", array("id" => Rahmad($_POST['delete']), "client" => $Menu['id'], "users" => $admins));
