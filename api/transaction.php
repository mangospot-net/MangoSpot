...
if(isset($_GET['price'])){
    ...
    $price = $Bsk->Tampil("packet", "price", "identity = '$Menu[identity]' and users = '$Menu[id]' and id = '$idprc' and status = 'true'");
...
if(isset($_GET['detail'])){
    ...
    $query_detail = $Bsk->Tampil("payment", "*", "id = '$id_detail' and identity = '$Menu[identity]' and users = '$Menu[id]'");
...
if(isset($_POST['packet'])){
    ...
    unset($_POST['id']);
    $check_post = $Bsk->Tampil("payment", "id", "id = '$id_post' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $query_post = ($check_post ? 
        $Bsk->Ubah("payment", $_POST, "id = '$check_post[id]'") : 
        $Bsk->Tambah("payment", 
            array_merge(
                $_POST, 
                array(
...
if(isset($_POST['active'])){
    ...
    $check_active = $Bsk->Tampil("payment", "id, packet, client, total, status", "id = '$id_active' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $check_packet = $Bsk->Tampil("packet", "id, groupname, price, total, voucher", "id = '$check_active[packet]' and identity = '$Menu[identity]' and users = '$Menu[id]' and status = 'true'");
    ...
    if($query_active){
        $Bsk->Ubah("payment", array("approve" => date('Y-m-d H:i:s')), "id = '$check_active[id]'");
        $Bsk->Ubah("packet", array("voucher" => $voucher), "id = '$check_packet[id]' ");
    }
...
if(isset($_POST['delete'])){
    ...
    $check_delete = $Bsk->Tampil("payment", "packet, client, total", "id = '$id_delete' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $shows_packet = $Bsk->Tampil("packet", "groupname, price", "id = '$check_delete[packet]' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $query_delete = $Bsk->Hapus("payment", array("id" => $id_delete, "users" => $Menu['id']));
    if($query_delete){
