...
if(isset($_GET['data'])){
    $nas = DataTable(
        "packet a inner join users b on a.client = b.id", 
        "a.id, b.name as client, a.groupname, a.price, a.total, a.voucher, a.defaults as default, a.status", 
...
if(isset($_GET['detail'])){
    ...
    $query_detail = $Bsk->Tampil("packet", "*", "id = '$id_detail' and identity = '$Menu[identity]' and users = '$Menu[id]'");
...
if(isset($_POST['client'])){
    ...
    unset($_POST['id']);
    $check_nast = $Bsk->Tampil("packet", "id", "client = '".Rahmad($_POST['client'])."' and groupname = '".Rahmad($_POST['groupname'])."' ");
    $check_post = $Bsk->Tampil("packet", "id", "id = '$id_post' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $data_post = array_replace(
        $_POST, 
        array("status" => (isset($_POST['status']) ? 'true' : 'false'))
    );
    $query_post = ($check_post ? 
        $Bsk->Ubah("packet", $data_post, "id = '$check_post[id]'") : ($check_nast ? false :
        $Bsk->Tambah("packet", array_merge($data_post, array("identity" => $Menu['identity'], "users" => $Menu['id']))))
    );
...
if(isset($_POST['active'])){
    ...
    $check_active = $Bsk->Tampil("packet", "id, status", "id = '$id_active' and identity = '$Menu[identity]' and users = '$Menu[id]'");
...
if(isset($_POST['default'])){
    ...
    $check_default = $Bsk->Tampil("packet", "id, client", "id = '$id_default' and identity = '$Menu[identity]' and users = '$Menu[id]'");
    $query_default = $Bsk->Ubah(
...
if(isset($_POST['delete'])){
    $query_delete = $Bsk->Hapus("packet", array("id" => Rahmad($_POST['delete']), "users" => $Menu['id']));
