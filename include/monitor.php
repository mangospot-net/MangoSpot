    ...
    static function getTemp() {
	 $obj = new stdClass();
        ...
        if ($return_val !== 0) {
            ...
            $obj->command = $cmd;
            $obj->percent = 0;
            $obj->percent = round($output[0]/1000, 2);
//            $obj->percent = intval(self::getServerLoad());
    ...
    static function getMemory() {
        ...
        if ($return_val !== 0) {
            ...
            if (preg_match("/Mem: *([0-9]+) *([0-9]+) *([0-9]+) */i", $output[1], $match)) {
                ...
                $obj->title = "Total: {$obj->memTotal} | Used: {$obj->memUsed} | Free: {$obj->memFree}";
            }
        }
        return $obj;
    }
    ...
    static function getDisk() {
        ...
        $cmd = "df -h";
        exec($cmd . "  2>&1", $output, $return_val);
        $diskfree  = self::humanFileSize(disk_free_space("."));
        $disktotal = self::humanFileSize(disk_total_space("."));
        $diskused  = self::humanFileSize(disk_total_space(".") - disk_free_space("."));
        if ($return_val !== 0) {
            ...
            $obj->diskUsed = $diskused;
            $obj->diskFree = $diskfree;
            $obj->title = "Total: {$disktotal} | Used: {$diskused} | Free: {$diskfree}";
            $obj->success = 1;
            $obj->output = $output;
            $obj->command = $cmd;
        }
